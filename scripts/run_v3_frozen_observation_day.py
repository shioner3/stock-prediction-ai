...
PREDICTIONS_LOG_PATH = STATE_ROOT / "predictions_log.jsonl"
REALIZED_RETURNS_LOG_PATH = STATE_ROOT / "realized_returns_log.jsonl"
...
def main() -> None:
    ...
    n_new_predictions = append_prediction_entries(PREDICTIONS_LOG_PATH, result.entries)
    ...
    # every not-yet-realized prediction against it.
    ...
    all_predictions = load_all_entries(PREDICTIONS_LOG_PATH)
    already_realized = load_existing_keys(REALIZED_RETURNS_LOG_PATH)
    realized_entries = compute_realized_return_entries(
        all_predictions, augmented_now, already_realized
    )
    n_new_realized = append_realized_return_entries(REALIZED_RETURNS_LOG_PATH, realized_entries)
